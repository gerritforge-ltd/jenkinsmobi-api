 ...
 * <p>
 * DynamicLists are always mapped as singletons in Guice. Sets store Providers
...
public class DynamicList<T> implements Iterable<T> {
  /**
   * Declare a singleton {@code DynamicList<T>} with a binder.
   * <p>
   * Sets must be defined in a Guice module before they can be bound:
   *
   * <pre>
   * DynamicList.setOf(binder(), Interface.class);
   * DynamicList.bind(binder(), Interface.class).to(Impl.class);
   * </pre>
   *
   ...
   *
   * <pre>
   * DynamicList.setOf(binder(), new TypeLiteral&lt;Thing&lt;Foo&gt;&gt;() {});
   * </pre>
   *
   * @param binder a new binder created in the module.
   * @param member type of entry in the list.
        ...
        while (next == null && itr.hasNext()) {
          ...
          if (p != null) {
            try {
              ...
              // TODO Log failed member of DynamicList.
            }
          }
        }
        return next != null;
