...
public class PluginRequestCommand implements RequestCommand {
  ...
  @Override
  public AbstractNode process(final Account account, final HttpServletRequest request) throws IOException {
    ...
    return getResponseLevelAtDepth(dispatchToPlugin(account, new WrappedHttpRequest(request, getPluginBaseRequestPath(request), "/"), path, plugin, config), path);
  }

  private String getPluginBaseRequestPath(HttpServletRequest request) {
    return "/" + new UrlPath(request)
            .getPluginId() + "$";
  }

  private AbstractNode getResponseLevelAtDepth(final AbstractNode response, final UrlPath path) throws ResourceNotFoundException {
    if (path.getDepth() != UrlPath.INFINITE_DEPTH && !response.isLeaf() && response instanceof ItemNode) {
  ...
  private AbstractNode dispatchToPlugin(final Account account, final HttpServletRequest request, final UrlPath urlPath, final Plugin pluginInstance, final PluginConfig pluginConf) throws IOException {
    AbstractNode response = pluginInstance.processRequest(account, request, pluginConf);

    if (response == null) {
      throw new ResourceNotFoundException("Plugin " + pluginInstance + " did not produce any response");
    }
