...
public class ProxyRequestCommand implements RequestCommand {
  ...
  @Inject
  public ProxyRequestCommand(final HttpClientURLDownloader urlDownloader,
      final ProxyUtil proxyUtil, final Config config) {
  ...
  public AbstractNode process(final Account account,
    ...
    if (url == null) {
      throw new ResourceNotFoundException(
          "Cannot find web parameter pointing to the target URL to proxyfy");
    }
    ...
    final RawBinaryNode result =
        urlDownloader.internalQueryForDownload(req, url, account,
            account.getPluginConfig(pluginName));

    if (isResultToBeProxied(result)) {
      final String resultString =
          proxyUtil.proxyfy(req.getHeader("User-Agent"), pluginName, url,
              requestURL.toString(), req.getParameter("decorate"),
  ...
  private String getCanonicalRequestURL(final HttpServletRequest req)
    ...
    if (config.getCanonicalUrl() != null) {
      ...
      requestURL =
          config.getCanonicalUrl()
              + origUrlPath.substring(reqContextPath.length());
    }
    return requestURL;
  }
}
