...
public class AssemblaSsoHandler implements SsoHandler {

  private static final HashMap<FormId, String> ssoFormIds =
      new HashMap<FormId, String>();
  public static void init() {
    ssoFormIds.put(FormId.LOGIN_FORM, "login-box");
    ssoFormIds.put(FormId.USER, "user_login");
    ssoFormIds.put(FormId.PASS, "user_password");
    JenkinsFormAuthHttpClient.registerSsoHander("assembla.com", new AssemblaSsoHandler());
  }

  @Override
  public IOException getException(final HttpResponse response) {
    return new IOException("Assembla Login Failed");
  }
