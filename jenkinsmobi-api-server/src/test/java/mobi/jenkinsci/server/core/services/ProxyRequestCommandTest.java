...
public class ProxyRequestCommandTest {
  ...
  private static final String A_REMOTE_RESOURCE_URL = "http://somewhere.com/some/remote/resource.html";
  ...
  private static final String A_REQUEST_URL = "http://host.com" + A_REQUEST_CONTEXT_PATH + A_REQUEST_PATH;
  ...
  public void setUp() throws IOException {
    when(mockUrlDownloader
            .download(any(HttpServletRequest.class), any(String.class), any(Account.class), any(PluginConfig.class)))
            .thenReturn(mockResponseNode);
  }
  ...
  private void mockRequestWithWebProxyParameter() {
    ...
    when(mockRequest.getRequestURL()).thenReturn(new StringBuffer(A_REQUEST_URL));
  ...
  public void shouldThrowResourceNotFoundExceptionWhenRequestDoesNotContainRemoteResourceUrl() throws IOException {
    proxyRequestCommand.process(mockAccount, mockRequest);
  }

  @Test
  public void whenRequestToProxyRemoteSourceShouldFetchFromRemoteUrl() throws IOException {
    mockRequestWithWebProxyParameter();

    proxyRequestCommand.process(mockAccount, mockRequest);

    verify(mockUrlDownloader).download(any(HttpServletRequest.class), eq(A_REMOTE_RESOURCE_URL), any(Account.class),
            any(PluginConfig.class));
  }

  @Test
  public void whenRequestRemoteHtmlSourceShouldProxyRemoteResourceContent() throws IOException {
    mockRequestWithWebProxyParameter();
    when(mockResponseNode.getHttpContentType()).thenReturn("text/html");

    proxyRequestCommand.process(mockAccount, mockRequest);

    verify(mockProxyUtil)
            .proxyfy(any(String.class), any(String.class), eq(A_REMOTE_RESOURCE_URL), any(InputStream.class));
  }
  ...
  public void shouldNotProxyNonHtmlRemoteResources() throws IOException {
    ...
    verify(mockProxyUtil, never())
            .proxyfy(any(String.class), any(String.class), any(String.class), any(InputStream.class));
  }

  @Test
  public void shouldUseCanonicalUrlInRewrittenResourceLinks() throws IOException {
    mockRequestWithWebProxyParameter();
    when(mockResponseNode.getHttpContentType()).thenReturn("text/html");
    ...
    verify(mockProxyUtil).rewriteLinks(eq(A_REMOTE_RESOURCE_URL), eq(A_CANONICAL_URL + A_REQUEST_PATH), anyString());
